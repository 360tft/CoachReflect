const CACHE_NAME = 'coachreflect-v1';
...
self.addEventListener('push', (event) => {
  ...
  try {
    ...
    const options = {
      body: data.body || 'New notification from CoachReflection',
    ...
    event.waitUntil(
      self.registration.showNotification(data.title || 'CoachReflection', options)
    );
  } catch (error) {
...
self.addEventListener('fetch', (event) => {
  ...
  // Skip JS and CSS files - always fetch fresh to prevent hydration issues
