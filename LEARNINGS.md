...
### Patterns
- Promo expiry kill switch (`isPromoActive()`) controlling all promo surfaces from one function — prevents stale promotions displaying after deadline without needing per-component cleanup
- UTM source tracking through sessionStorage passthrough to Stripe checkout metadata enables email-to-conversion attribution — tracks which email drove which upgrade
- Admin promo conversion stats (emails sent vs upgraded) on the dashboard gives immediate feedback on promo effectiveness without needing external analytics
- Avatar-based product review ("rate this as a football coach") as a structured QA methodology — multiple passes (player avatar, coach avatar) surface different UX gaps

### Gotchas
- Reducing free tier limits (2→1 reflections/day) requires updating the limit in many places — 13 files touched for a single number change indicates the limit constant isn't truly centralised everywhere it's referenced

### Decisions
- CoachReflect migrated from managed hosting to self-hosted platform — reduces hosting costs for a product still building revenue
- Free tier reduced from 2 to 1 reflection/day — tighter free limits paired with time-limited 50% off promo (expires 7 March) create stronger conversion pressure
- Pro+ upsell link added to upgrade modal — surfaces the higher tier to users already considering an upgrade

## 2026-02-19

### Patterns
- Admin-triggered promo email blast with preview recipient count, dry run mode, and double-send protection prevents accidental mass re-sends — the admin dashboard shows exactly who will receive the email before hitting send
- Strikethrough pricing on upgrade modal and pricing page (showing original price crossed out next to discounted price) creates urgency — more compelling than just showing the discount percentage
- Stripe coupon auto-applied at checkout via `STRIPE_ANNUAL_PROMO_COUPON_ID` env var — no user action needed, the discount appears automatically on annual plans

### Gotchas
- Promo email reply-to must be set to a monitored mailbox (admin@360tft.com) — default Resend from-address (hello@send.coachreflection.com) has no inbox, so user replies go nowhere

### Decisions
- 50% off first year on annual individual plans only (not team/club) using Stripe coupon auto-applied at checkout — monthly plans and team plans excluded
- CLOSER framework applied to promo email template — structures the email around clarifying pain, labelling the problem, and selling the outcome rather than listing features
- Promo email targets free users only (never subscribed) — existing subscribers don't see the discount to avoid cannibalising current revenue

## 2026-02-17

### Patterns
- NativeOnly component (inverse of NativeHidden) provides a clean pattern for showing IAP-specific UI only on iOS/Android — use both NativeHidden and NativeOnly to maintain separate web and native upgrade paths in the same codebase
- Apple App Store Guidelines 2.1 and 3.1.1 rejection: ANY reference to Stripe checkout visible to iOS users triggers rejection — wrap ALL payment-related UI (billing toggle, club plans, promotional upgrade cards, price mentions) in NativeHidden, not just the checkout button
- Docker/Coolify deployment: same standalone output + multi-stage Dockerfile pattern as FootballGPT — enables consistent containerised deployment across all products

### Gotchas
- Apple rejected because Stripe checkout was exposed to iOS users in multiple places (settings billing toggle, dashboard upgrade cards, analytics button with hardcoded "$7.99/mo", players list upgrade link) — a single NativeHidden wrapper on the checkout button isn't enough; audit EVERY screen for payment references
- WhatsApp voice messages are .opus format which may not be supported by the Whisper transcription API — need to handle audio format conversion or reject unsupported formats with a clear error message
- App Store compliance requires routing ALL upgrade flows through RevenueCat IAP on native, including subtle upgrade prompts in analytics, player lists, and CPD export — not just the main upgrade modal

### Decisions
- Created NativeOnly component and wrapped all payment UI appropriately for App Store resubmission — Stripe hidden on native, RevenueCat IAP shown instead
- Players list upgrade flow changed from direct Link to UpgradeModal for proper IAP routing on native
- CPD export upgrade link changed to point to /dashboard/settings instead of /pricing to avoid exposing web pricing to native users

## 2026-02-16

### Patterns
- AI task extraction from chat insights: extend the existing `extract` endpoint to also pull `ai_action_items` — turns passive reflections into active to-do items without coaches needing to manually create tasks
- Action items checklist on reflection detail page creates a feedback loop: reflect on session → AI extracts tasks → coach ticks them off → gamification badge earned
- Task reminder emails (daily cron, Pro/Pro+ only) keep coaches engaged between reflection sessions — the app reaches out to them rather than waiting for them to return
- Tasks as a Pro differentiator: free users can see AI-extracted tasks but reminders and full task management are gated — gives free users a taste of the productivity loop

### Gotchas
- AI insight extraction was returning empty `ai_action_items` array because the extraction prompt didn't explicitly request action items — the AI only extracts what you ask for in the structured output schema
- Task migration needs both the `tasks` table AND updates to the insight extraction prompt and reflection analysis route — shipping the table without updating the AI extraction means no tasks ever get created

### Decisions
- Tasks feature shipped with AI extraction from both chat and reflections, dashboard widget, full tasks page, completion tracking with gamification badges, and daily reminder emails

## 2026-02-15

### Patterns
- Auto-zoom drill diagrams: calculate bounding box of all elements and zoom in when they use less than 60% of canvas space — prevents drills from clustering in one corner when AI generates coordinates in a narrow range
- AI drill coordinate spreading: update system prompt to instruct the AI to spread coordinates across the full 0-100 range — fixes the root cause alongside the renderer auto-zoom safety net

### Gotchas
- Drill diagrams rendering too small wasn't a canvas size issue — it was the AI generating all coordinates within a 20-30 unit range instead of using the full 0-100 coordinate space. Fix requires both prompt-side (spread coordinates) and renderer-side (auto-zoom fallback)

### Decisions
- Two-layer fix for drill diagram sizing: AI prompt updated to spread coordinates AND renderer auto-zooms when elements are clustered — belt and braces approach prevents recurrence
...
### Gotchas
- SEO page titles had "CoachReflection" doubled (e.g. "Football Reflection | CoachReflection | CoachReflection") — meta tag templates that append the brand name must check whether the page title already includes it
